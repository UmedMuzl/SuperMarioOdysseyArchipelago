...
from Packets import PacketHeader, PacketType, Packet, ConnectionType
from Data import  shop_items, inverse_shop_items, get_item_type, inverse_moon_items
        ...
        while True:
            ...
            if collect in inverse_shop_items or collect in shop_items:
                if collect.isnumeric():
                    ...
                    writer.write(packet.serialize())
                    await writer.drain()
            if collect in inverse_moon_items or collect in inverse_moon_items.values():
                if collect.isnumeric():
                    collect = int(collect)
