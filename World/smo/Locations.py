from BaseClasses import Location

class SMOLocation(Location):
    game: str = "Super Mario Odyssey"

# Cap
loc_Cap = {
    "Frog-jumping above the Fog": 1019,
    "Frog-jumping from the Top Deck": 815,
    "Cap Kingdom Timer Challenge 1": 861,
    "Good Evening, Captain Toad!": 227,
    "Shopping in Bonneton": 230,
    "Skimming the Poison Tide": 236,
    "Slipping through the Poison Tide": 237,
    "Push-Block Peril": 234,
    "Hidden Among the Push-Blocks": 235,
    "Searching the Frog Pond": 238,
    "Secrets of the Frog Pond": 239,
}

loc_Cap_Shop = {
    "Cap Kingdom Sticker": 2506,
    "Plush Frog": 2507,
    "Bonneton Tower Model": 2508,
    "Black Top Hat": 2504,
    "Black Tuxedo": 2505
}

loc_Cap_Postgame = {
    "The Forgotten Treasure": 228,
    "Taxi Flying Through Bonneton": 1073,
    "Bonnetter Blockade": 1038,
    "Cap Kingdom Regular Cup": 1033,
    "Peach in the Cap Kingdom": 229,
    "Found with Cap Kingdom Art": 1086,
    "Next to Glasses Bridge": 233,
    "Danger Sign": 816,
    "Under the Big One's Brim": 817,
    "Fly to the Edge of the Fog": 1158,
    "Spin the Hat, Get a Prize": 1025,
    "Hidden in a Sunken Hat": 231,
    "Fog-Shrouded Platform": 813,
    "Bird Traveling in the Fog": 1156,
    "Caught Hopping Near the Ship!": 232,
    "Taking Notes: In the Fog": 1018,
    "Cap Kingdom Timer Challenge 2": 981,
    "Cap Kingdom Master Cup": 1034,
    "Roll On and On": 950,
    "Precision Rolling": 951
}


# Cascade
loc_Cascade = {
    "Our First Power Moon" : 205,
    "Multi Moon Atop the Falls": 218,
    "Chomp Through the Rocks": 206,
    "Behind the Waterfall": 212,
    "On Top of the Rubble": 1145,
    "Treasure of the Waterfall Basin": 216,
    "Above a High Cliff": 210,
    "Across the Floating Isles": 208,
    "Cascade Kingdom Timer Challenge 1": 669,
    "Cascade Kingdom Timer Challenge 2": 670,
    "Good Morning, Captain Toad!": 204,
    "Dinosaur Nest: Big Cleanup!": 1116,
    "Dinosaur Nest: Running Wild!": 1115,
    "Nice Shot with the Chain Chomp!": 225,
    "Very Nice Shot with the Chain Chomp!": 226,
    "Past the Chasm Lifts": 221,
    "Hidden Chasm Passage": 222
}

loc_Cascade_Revisit = {
    "Rolling Rock by the Falls": 1004,
    "Shopping in Fossil Falls": 211,
}

loc_Cascade_Shop = {
    "Caveman Headwear": 2509,
    "Caveman Outfit": 2510,
    "Cascade Kingdom Sticker": 2511,
    "T-Rex Model": 2512,
    "Triceratops Trophy": 2513
}

loc_Cascade_Post_Metro = {
    "A Tourist in the Cascade Kingdom!": 906
}

loc_Cascade_Post_Snow = {
    "Secret Path to Fossil Falls!": 207
}

loc_Cascade_Postgame = {
    "Peach in the Cascade Kingdom": 209,
    "Cascade Kingdom Regular Cup": 708,
    "Caveman Cave-Fan": 866,
    "Sphynx Traveling to the Waterfall": 1041,
    "Bottom of the Waterfall Basin": 213,
    "Just a Hat, Skip, and a Jump": 826,
    "Treasure Under the Cliff": 214,
    "Next to the Stone Arch": 215,
    "Guarded by a Colossal Fossil": 825,
    "Under the Old Electrical Pole": 823,
    "Under the Ground": 822,
    "Inside the Busted Fossil": 1167,
    "Caught Hopping at the Waterfall!": 217,
    "Taking Notes: Hurry Upward": 770,
    "Across the Mysterious Clouds": 894,
    "Cascade Kingdom Master Cup": 224,
    "Atop a Wall Among the Clouds": 223,
    "Across the Gusty Bridges": 673,
    "Flying Far Away from Gusty Bridges": 674
}

# Sand
loc_Sand = {
    "Atop the Highest Tower": 497,
    "Moon Shards in the Sand": 496,
    "Overlooking the Desert Town": 517,
    "Alcove in the Ruins": 518,
    "On the Leaning Pillar": 516,
    "Hidden Room in the Flowing Sands": 523,
    "Secret of the Mural": 505,
    "On Top of Stone Archway": 520,
    "From a Crate In The Ruins": 507,
    "On the Lone Pillar": 522,
    "Where the Birds Gather": 511,
    "Top of a Dune": 506,
    "Lost in the Luggage": 510,
    "Bullet Bill Breakthrough": 519,
    "Inside a Block Is a Hard Place": 508,
    "Bird Traveling the Desert": 526,
    "The Treasure of Jaxi Ruins": 530,
    "Desert Gardening: Plaza Seed": 1112,
    "Desert Gardening: Ruins Seed": 1113,
    "Desert Gardening: Seed on the Cliff": 1114,
    "Taking Notes: Jump on the Palm": 525,
    "Herding Sheep in the Dunes": 532,
    "Among the Five Cactuses": 893,
    "Wandering Cactus": 509,
    "Sand Quiz: Wonderful!": 498,
    "Shopping in Tostarena": 565,
    "Employees Only": 564,
    "Sand Kingdom Slots": 1047,
    "Walking the Desert!": 533,
    "Ice Cave Treasure": 567,
    "Sphynx's Treasure Vault": 566,
    "A Rumble from the Sandy Floor": 1136,
    "Dancing with New Friends": 569,
    "The Invisible Maze": 562,
    "Skull Sign in the Transparent Maze": 561,
    "The Bullet Bill Maze: Break Through!": 555,
    "The Bullet Bill Maze: Side Path": 554,
    "Jaxi Driver": 557,
    "Jaxi Stunt Driving": 556,
}

loc_Sand_Pyramid = {
    "Showdown on the Inverted Pyramid": 495,
    "Secret of the Inverted Mural": 504,
    "On the Statue's Tail": 513,
    "Hidden Room in the Inverted Pyramid": 563,
}

loc_Sand_Underground = {
    "The Hole in the Desert": 560,
    "Underground Treasure Chest": 953,
    "Goomba Tower Assembly": 558,
}

loc_Sand_Peace = {
    "Hang Your Hat on the Fountain": 534,
    "Found in the Sand! Good Dog!": 512,
    "Sand Kingdom Timer Challenge 1": 531,
    "Sand Kingdom Timer Challenge 2": 536,
    "Sand Kingdom Timer Challenge 3": 539,
    "Fishing in the Oasis": 502,
    "Love in the Heart of the Desert": 493,
    "You're Quite a Catch, Captain Toad!": 692,
    "Jaxi Reunion!": 1059,
    "Welcome Back, Jaxi!": 501,
    "Bird Traveling Wastes": 552,
    "The Lurker Under the Stone": 538,
    "Under the Mummy's Curse": 995,
    "Strange Neighborhood": 570,
    "Above a Strange Neighborhood": 571
}

loc_Sand_Shop = {
    "Captain's Hat": 2503,
    "Sombrero": 2514,
    "Poncho": 2515,
    "Cowboy Hat": 2516,
    "Cowboy Outfit": 2517,
    "Sand Kingdom Sticker": 2518,
    "Jaxi Statue": 2519,
    "Inverted Pyramid Model": 2520,
    "Employee Cap": 2585,
    "Employee Uniform": 2586,
    "Boxer Shorts": 2587
}

loc_Sand_Postgame = {
    "Found with Sand Kingdom Art": 1096,
    "Jammin' in the Sand Kingdom": 910,
    "Hat-and-Seek: In the Sand": 924,
    "Sand Kingdom Regular Cup": 1045,
    "Binding Band Returned": 887,
    "Round-the-World Tourist": 535,
    "Peach in the Sand Kingdom": 544,
    "Mighty Leap from the Palm Tree!": 547,
    "On the North Pillar": 546,
    "Into the Flowing Sands": 799,
    "In the Skies Above the Canyon": 550,
    "Island in the Poison Swamp": 521,
    "An invisible Gleam": 542,
    "On the Eastern Pillar": 540,
    "Caught Hopping in the Desert!": 553,
    "Poster Cleanup": 545,
    "Taking Notes: Running Down": 515,
    "Taking Notes: In the Wall Painting": 783,
    "Love at the Edge of the Desert": 965,
    "More Walking in the Desert": 712,
    "Sand Kingdom Master Cup": 1046,
    "Where the Transparent Platforms End": 572,
    "Jump Onto the Transparent Lift": 573,
    "Colossal Ruins: Dash Jump!": 575,
    "Sinking Colossal Ruins: Hurry!": 574,
    "Through the Freezing Waterway": 35,
    "Freezing Waterway: Hidden Room": 36
}

loc_Sand_Revisit = {
    "Secret Path to Tostarena!": 524,
}

# Lake
loc_Lake = {
    "Broodals Over the Lake": 424,
    "Dorrie-Back Rider": 411,
    "Cheep Cheep Crossing": 412,
    "End of the Hidden Passage": 410,
    "What's in the Box?": 404,
    "On the Lakeshore": 401,
    "From the Broken Pillar": 405,
    "Treasure in the Spiky Waterway": 415,
    "Lake Gardening: Spiky Passage Seed": 1166,
    "Lake Kingdom Timer Challenge 1": 715,
    "Lake Kingdom Timer Challenge 2": 420,
    "Moon Shards in the Lake": 413,
    "Taking Notes: Dive and Swim": 402,
    "Taking Notes: In the Cliffside": 407,
    "Lake Fishing": 416,
    "I Met a Lake Cheep Cheep!": 409,
    "Our Secret Little Room": 414,
    "Let's Go Swimming, Captain Toad!": 403,
    "Shopping in Lake Lamode": 430,
    "A Successful Repair Job": 971,
    "I Feel Underdressed": 406,
    "Unzip the Chasm": 1008,
    "Super-Secret Zipper": 1009,
    "Jump, Grab, Cling, and Climb": 425,
    "Jump, Grab, and Climb Some More": 426,
    "Found with Lake Kingdom Art": 1094,
}

loc_Lake_Shop = {
    "Swim Goggles": 2521,
    "Swimwear": 2522,
    "Lake Kingdom Sticker": 2523,
    "Rubber Dorrie": 2524,
    "Underwater Dome": 2525,
    "Fashionable Cap": 2588,
    "Fashionable Outfit": 2589
}

loc_Lake_Post_Seaside = {
    "Secret Path to Lake Lamode!": 417
}

loc_Lake_Postgame = {
    "Taxi Flying Through Lake Lamode": 1078,
    "That Trendy “Pirate” Look": 870,
    "Space Is “In” Right Now": 869,
    "That “Old West” Style": 871,
    "Lake Kingdom Regular Cup": 709,
    "Peach in the Lake Kingdom": 408,
    "Behind the Floodgate": 422,
    "High-Flying Leap": 771,
    "Deep, Deep Down": 719,
    "Rooftop of the Water Plaza": 1015,
    "Bird Traveling Over the Lake": 423,
    "Love by the Lake": 1017,
    "Lake Kingdom Master Cup": 720,
    "Waves of Poison: Hoppin' Over": 434,
    "Waves of Poison: Hop to It!": 431
}

loc_Wooded = {
    "Road to Sky Garden": 129,
    "Rolling Rock in the Woods": 149,
    "Caught Hopping in the Forest!": 148,
    "Atop a Tall Tree": 139,
    "Tucked Away Inside a Tunnel": 143,
    "The Nut ‘Round The Corner": 145,
    "Climb the Cliff to Get the Nut": 140,
    "The Nut in the Red Maze": 141,
    "The Nut at the Dead End": 142,
    "Fire in the Cave": 136,
    "Shopping in Steam Gardens": 138,
    "Nut Planted in the Tower": 180,
    "Stretching Your Legs": 179,
    "Rolling Rock in the Deep Woods": 183,
    "Glowing in the Deep Woods": 1137,
    "Past The Peculiar Pipes": 1159,
    "By the Babbling Brook in the Deep Woods": 185,
    "The Hard Rock in Deep Woods": 186,
    "A Treasure Made of Coins": 1153,
    "Beneath the Roots of a Moving Tree": 184,
    "Deep Woods Treasure Trap": 188,
    "Exploring for Treasure": 187,
    "Flooding Pipeway": 196,
    "Flooding Pipeway Ceiling Secret": 197,
}

loc_Wooded_Post_Metro = {
    "Secret Path to the Steam Gardens": 137
}

loc_Wooded_Post_Story1 = {
    "Flower Thieves of Sky Garden": 130,
    "Path to the Secret Flower Field": 159,
    "Cracked Nut on a Crumbling Tower": 144,
    "The Nut that Grew on the Tall Fence": 147,
    "Love in the Forest Ruins": 131,
    "Thanks for the Charge!": 135,
    "Spinning-Platforms Treasure": 195,
    "Over the Cliff's Edge": 146,
    "Behind the Rock Wall": 134,
    "Back Way Up the Mountain": 150,
    "Flower Road Run": 191,
    "Flower Road Reach": 192,
    "Elevator Escalation": 190,
    "Elevator Blind Spot": 189
}

loc_Wooded_Peace = {
    "Defend the Secret Flower Field!": 181,
    "Inside the Rock in the Forest": 1026,
    "Hey Out There, Captain Toad!": 133,
    "Make the Secret Flower Field Bloom": 182,
    "Wooded Kingdom Timer Challenge 1": 157,
    "Wooded Kingdom Timer Challenge 2": 676,
    "Wandering in the Fog": 193,
    "Nut Hidden in the Fog": 194,
    "Walking on Clouds": 198,
    "Above the Clouds": 199,
    "Found with Wooded Kingdom Art": 1089
}

loc_Wooded_Shop = {
    "Explorer Hat": 2526,
    "Explorer Outfit": 2527,
    "Scientist Visor": 2528,
    "Scientist Outfit": 2529,
    "Wooded Kingdom Sticker": 2530,
    "Flowers from Steam Gardens": 2531,
    "Steam Gardener Watering Can": 2532,
    "Mechanic Cap": 2590,
    "Mechanic Outfit": 2591
}

loc_Wooded_Postgame = {
    "Swing Around Secret Flower Field": 1011,
    "Jammin' in the Wooded Kingdom": 905,
    "Wooded Kingdom Regular Cup": 153,
    "Peach in the Wooded Kingdom": 155,
    "High Up in the Cave": 168,
    "Lost in the Tall Trees": 892,
    "Looking Down on the Goombas": 160,
    "High Up on a Rock Wall": 177,
    "The Nut in the Robot Storeroom": 169,
    "Above the Iron Mountain Path": 161,
    "The Nut Under the Observation Deck": 174,
    "Bird Traveling the Forest": 166,
    "Invader in the Sky Garden": 156,
    "Hot, Hot, Hot from the Campfire": 163,
    "Wooded Kingdom Timer Challenge 3": 1010,
    "Moon Shards in the Forest": 152,
    "Taking Notes: On Top of the Wall": 170,
    "Taking Notes: Stretching": 151,
    "Wooded Kingdom Master Cup": 901,
    "I Met an Uproot!": 1001,
    "Invisible Road: Danger!": 202,
    "Invisible Road: Hidden Room": 203,
    "Herding Sheep Above the Forest Fog": 200,
    "Herding Sheep on the Iron Bridge": 201,
    "Down and Back Breakdown Road": 882,
    "Below Breakdown Road": 883
}

loc_Cloud = {
    "Picture Match: Basically a Goomba" : 982
}

loc_Post_Cloud = {
    "Beat Bowser in Cloud": 2501
}

loc_Cloud_Postgame = {
    "Peach in the Cloud Kingdom": 1118,
    "Digging in the… Cloud?": 851,
    "High, High Above the Clouds": 852,
    "Crossing the Cloud Sea": 853,
    "Taking Notes: Up and Down": 1160,
    "Picture Match: A Stellar Goomba!": 983,
    "King of the Cube!": 914,
    "The Sixth Face": 915
}

loc_Lost = {
    "Atop a Propeller Pillar": 376,
    "Below the Cliff's Edge": 380,
    "Inside the Stone Cage": 374,
    "On a Tree in the Swamp": 377,
    "Over the Fuzzies, Above the Swamp": 384,
    "Avoiding Fuzzies Inside the Wall": 387,
    "Inside the Rising Stone Pillar": 375,
    "Enjoying the View of Forgotten Isle": 385,
    "On the Mountain Road": 379,
    "A Propeller Pillar's Secret": 373,
    "Wrecked Rock Block": 942,
    "A Butterfly's Treasure": 388,
    "Cave Gardening": 378,
    "Moon Shards in the Jungle": 386,
    "Peeking Out from Under the Bridge": 382,
    "Twist ‘n' Turn-Up Treasure": 1075,
    "Soaring Over the Forgotten Isle!": 381,
    "The Caged Gold": 383,
    "Get Some Rest, Captain Toad": 372,
    "Shopping on Forgotten Isle": 398,
}

loc_Lost_Revisit = {
    "Caught Hopping in the Jungle!": 390
}

loc_Lost_Shop = {
    "Aviator Cap": 2533,
    "Aviator Outfit": 2534,
    "Lost Kingdom Sticker": 2535,
    "Potted Palm Tree": 2536,
    "Butterfly Mobile": 2537
}

loc_Lost_Postgame = {
    "Taxi Flying Through Forgotten Isle": 1077,
    "I met a Tropical Wiggler!": 943,
    "Lost Kingdom Regular Cup": 832,
    "Peach in the Lost Kingdom": 394,
    "The Shining Fruit": 395,
    "Jump Down to the Top of a Tree": 396,
    "Line It Up, Blow It Up": 392,
    "Taking Notes: Stretch and Shrink": 393,
    "Lost Kingdom Master Cup": 897,
    "Lost Kingdom Timer Challenge": 855,
    "Stretch and Traverse the Jungle": 399,
    "Aglow in the Jungle": 400,
    "Chasing Klepto": 491,
    "Extremely Hot Bath": 492
}

loc_Metro = {
    "New Donk City's Pest Problem": 37,
    "Drummer on Board!": 42,
    "Guitarist on Board!": 41,
    "Bassist on Board!": 44,
    "Trumpeter on Board!": 43,
    "Inside an Iron Girder": 38,
    "Swaying in the Breeze": 39,
    "Girder Sandwich": 54,
    "Glittering Above the Pool": 65,
    "Dizzying Heights": 61,
    "Secret Girder Tunnel!": 59,
    "Who Piled Garbage on This?": 49,
    "Hidden in the Scrap": 62,
    "Left at the Cafe?": 77,
    "How Do They Take Out the Trash?": 68,
    "City Gardening: Building Planter": 70,
    "City Gardening: Plaza Planter": 71,
    "City Gardening: Rooftop Planter": 69,
    "How You Doin', Captain Toad?": 46,
    "Free Parking: Rooftop Hop": 809,
    "Bench Friends": 50,
    "Shopping in New Donk City": 101,
    "Metro Kingdom Slots": 1040,
    "Jump-Rope Hero": 60,
    "Jump-Rope Genius": 51,
    "Remotely Captured Car": 53,
    "RC Car Pro!": 105,
    "Taking Notes: In the Private Room": 102,
    "City Hall Lost & Found": 100,
    "Pushing Through the Crowd": 1022,
    "High Over the Crowd": 1023,
    "Rewiring the Neighborhood": 122,
    "Off the Beaten Wire": 121,
    "Moon Shards Under Siege": 114,
    "Sharpshooting Under Siege": 113,
    "Inside the Rotating Maze": 118,
    "Outside the Rotating Maze": 119,
    "Hanging from a High-Rise": 103,
    "Vaulting Up a High-Rise": 104,
    "Bullet Billding": 115,
    "One Man's Trash…": 116,
    "Motor Scooter: Escape!": 1139,
    "Big Jump: Escape!": 1140,
    "Found with Metro Kingdom Art": 1088
}

loc_Metro_Post_Sand = {
    "Secret Path to New Donk City!": 40
}

loc_Metro_Sewer_Access = {
    "Powering Up the Station": 107,
    "Sewer Treasure": 1100,
    "A Traditional Festival!": 95,
}

loc_Metro_Peace = {
    "Celebrating in the Streets!": 875,
    "Caught Hopping on a Building!": 99,
    "Metro Kingdom Timer Challenge 1": 1054,
    "Metro Kingdom Timer Challenge 2": 839,
    "A Tourist in the Metro Kingdom!": 881
}

loc_Metro_Shop = {
    "Builder Helmet": 2538,
    "Builder Outfit": 2539,
    "Golf Cap": 2540,
    "Golf Outfit": 2541,
    "Metro Kingdom Sticker": 2542,
    "New Donk City Hall Model": 2543,
    "Pauline Statue": 2544,
    "Black Fedora": 2592,
    "Black Suit": 2593
}

loc_Metro_Postgame = {
    "Bird Traveling in the City": 1150,
    "Mario Signs His Name": 97,
    "Surprise Clown!": 874,
    "A Request from the Mayor": 96,
    "Jammin' in the Metro Kingdom": 904,
    "Sphynx in the City": 1037,
    "Free Parking: Leap of Faith": 979,
    "Metro Kingdom Regular Cup": 939,
    "Hat-and-Seek: In the City": 81,
    "Powering Up the Power Plant": 848,
    "Up on the Big Screen": 928,
    "Down Inside the Big Screen": 929,
    "Peach in the Metro Kingdom": 75,
    "Hanging Between Buildings": 84,
    "Crossing Lines": 78,
    "Out of a Crate in the City": 74,
    "Bird Traveling in the Park": 1155,
    "Metro Kingdom Timer Challenge 3": 938,
    "Found in the Park! Good Dog!": 82,
    "RC Car Champ!": 106,
    "Metro Kingdom Master Cup": 940,
    "Hat-and-Seek: In the Crowd": 1021,
    "Scaling Pitchblack Mountain": 949,
    "Reaching Pitchblack Island": 948,
    "Swinging Scaffolding: Jump!": 125,
    "Swinging Scaffolding: Break!": 126,
    "Moto Scooter: Daredevil!": 123,
    "Full-Throttle Scooting!": 124
}

loc_Seaside = {
    "The Stone Pillar Seal": 438,
    "The Lighthouse Seal": 441,
    "The Hot Spring Seal": 440,
    "The Seal Above the Canyon": 439,
    "On the Cliff Overlooking the Beach": 449,
    "Ride the Jetstream": 455,
    "Ocean-Bottom Maze: Treasure": 466,
    "Ocean-Bottom Maze: Hidden Room": 467,
    "Underwater Highway Tunnel": 450,
    "Shh! It's a Shortcut!": 451,
    "Gap in the Ocean Trench": 458,
    "Slip Through the Nesting Spot": 459,
    "Merci, Dorrie!": 453,
    "Under a Dangerous Ceiling": 445,
    "What the Waves Left Behind": 447,
    "The Back Canyon: Excavate!": 444,
    "Bubblaine Northern Reaches": 1151,
    "Wriggling on the Sandy Bottom": 1024,
    "Glass Palace Treasure Chest": 1057,
    "Treasure Trap Hidden in the Inlet": 448,
    "Sea Gardening: Inlet Seed": 1103,
    "Sea Gardening Canyon Seed": 1104,
    "Sea Gardening: Hot-Spring Seed": 1106,
    "Sea Gardening: Ocean Trench Seed": 1105,
    "Seaside Kingdom Timer Challenge 1": 468,
    "Moon Shards in the Sea": 454,
    "Taking Notes: Ocean Surface Dash": 442,
    "Love by the Seaside": 446,
    "Good Job, Captain Toad!": 443,
    "Ocean Quiz: Good!": 457,
    "Shopping in Bubblaine": 460,
    "Looking Back in the Dark Waterway": 1107,
    "The Sphynx's Underwater Vault": 483,
    "A Rumble on the Seaside Floor": 1135,
    "A Relaxing Dance": 484,
    "Wading in the Cloud Sea": 485,
    "Sunken Treasure in the Cloud Sea": 486,
    "Fly Through the Narrow Valley": 478,
    "Treasure Chest in the Narrow Valley": 479,
    "Hurry and Stretch": 482,
    "Stretch on the Side Path": 481,
    "Found with Seaside Kingdom Art": 1095,
}

loc_Seaside_Peace = {
    "The Glass Is Half Full!": 437,
    "Bonjour, Dorrie!": 452,
    "Seaside Kingdom Timer Challenge 2": 863,
    "Found on the Beach! Good Dog!": 471,
    "Lighthouse Leaper": 474,
    "Beach Volleyball: Champ": 472,
    "Beach Volleyball: Hero of the Beach!": 473
}

loc_Seaside_Shop = {
    "Resort Hat": 2550,
    "Resort Outfit": 2551,
    "Sailor Hat": 2552,
    "Sailor Suit": 2553,
    "Seaside Kingdom Sticker": 2554,
    "Glass Tower Model": 2555,
    "Sand Jar": 2556,
    "Pirate Hat": 2594,
    "Pirate Outfit": 2595
}

loc_Seaside_Postgame = {
    "Secret Path to Bubblaine": 464,
    "Seaside Kingdom Regular Cup": 1028,
    "Peach in the Seaside Kingdom": 477,
    "Above the Parasol: Catch!": 780,
    "What Shines Inside the Glass": 779,
    "A Fine Detail on the Glass": 920,
    "Underwater Highway West: Explore!": 476,
    "Underwater Highway East: Explore!": 773,
    "Rapid Ascent on Hot Spring Island": 1043,
    "A Light Next to the Lighthouse": 776,
    "The Tall Rock Shell in the Deep Ocean": 778,
    "At the Base of the Lighthouse": 775,
    "Bird Traveling Over the Ocean": 1120,
    "Caught Hopping at Glass Palace!": 475,
    "Seaside Kingdom Timer Challenge 3": 864,
    "Taking Notes: Ocean-Bottom Maze": 790,
    "Taking Notes in the Sea": 1163,
    "Seaside Kingdom Master Cup": 1029,
    "Aim! Poke!": 488,
    "Poke! Roll!": 487,
    "The Spinning Maze: Search!": 489,
    "The Spinning Maze: Open!": 490
}

loc_Snow = {
    "The Icicle Barrier": 17,
    "The Ice Wall Barrier": 22,
    "The Gusty Barrier": 18,
    "The Snowy Mountain Barrier": 25,
    "Entrance to Shiveria": 1081,
    "Behind the Snowy Mountain": 21,
    "Shining in the Snow in Town": 1080,
    "Atop a Blustery Arch": 16,
    "The Shiverian Treasure Chest": 23,
    "Treasure in the Ice Wall": 24,
    "Ice-Dodging Goomba Stack": 20,
    "Captain Toad is Chilly!": 694,
    "Shopping in Shiveria": 868,
    "Moon Shards in the Cold Room": 1030,
    "Slip Behind the Ice": 1031,
    "Found with Snow Kingdom Art": 1087
}

loc_Snow_Peace = {
    "The Bound Bowl Grand Prix": 1020,
    "Caught Hopping in the Snow!": 1002,
    "Snow Kingdom Timer Challenge 1": 678,
    "Snow Kingdom Timer Challenge 2": 686,
    "Moon Shards in the Snow": 2,
    "Taking Notes: Snow Path Dash": 3,
    "Fishing in the Glacier!": 4,
    "I'm Not Cold!": 873,
    "Walking on Ice!": 877,
    "Snowline Circuit Class S": 879,
    "Dashing Over Cold Water!": 12,
    "Dashing Above and Beyond!": 13,
    "Jump ‘n' Swim in the Freezing Water": 14,
    "Freezing Water Near the Ceiling": 15,
    "Blowing and Sliding": 33,
    "Spinning Above the Clouds": 30,
    "High-Altitude Spinning": 831,
}

loc_Snow_Shop = {
    "Snow Hood": 2545,
    "Snow Suit": 2546,
    "Snow Kingdom Sticker": 2547,
    "Shiverian Rug": 2548,
    "Shiverian Nesting Dolls": 2549
}

loc_Snow_Postgame = {
    "Secret Path to Shiveria!": 937,
    "Snow Kingdom Regular Cup": 695,
    "Hat-and-Seek in the Snow": 903,
    "Peach in the Snow Kingdom": 19,
    "Shining on High": 8,
    "Above the Freezing Fish Pond": 6,
    "Ice Floe Swimming": 7,
    "Icy Jump Challenge": 703,
    "Forgotten in the Holding Room": 843,
    "It Popped Out of the Ice": 10,
    "Deep in the Cold, Cold Water": 9,
    "Water Pooling in the Crevasse": 701,
    "Squirming Under the Ice": 704,
    "Snow Kingdom Timer Challenge 3": 1003,
    "Stacked-Up Ice Climb": 699,
    "I Met a Snow Cheep Cheep!": 973,
    "Even More Walking on Ice!": 878,
    "Snow Kingdom Master Cup": 900,
    "Iceburn Circuit Class A": 998,
    "Iceburn Circuit Class S": 997,
    "Running the Flower Road": 31,
    "Looking Back on the Flower Road": 32
}

loc_Luncheon = {
    "The Broodals Are After Some Cookin'": 291,
    "Piled on the Salt": 256,
    "Lurking in the Pillar's Shadow": 250,
    "Overlooking a Bunch of Ingredients": 246,
    "Luncheon Kingdom Timer Challenge 1": 264,
    "Love Above the Lava": 803,
    "Magma Swamp: Floating and Sinking": 302,
    "Corner of the Magma Swamp": 303
}

loc_Luncheon_Post_Wooded = {
    "Secret Path to Mount Volbono!": 260
}

loc_Luncheon_Post_Spewart = {
    "Under the Cheese Rocks": 251,
    "Atop the Jutting Crag": 253,
    "Is This an Ingredient Too?!": 240,
    "Atop a Column in a Row": 247,
    "Island of Salt Floating in the Lava": 245,
    "Golden Turnip Recipe 1": 242,
    "Golden Turnip Recipe 2": 241,
    "Shopping in Mount Volbono": 294,
    "Luncheon Kingdom Slots": 1042,
    "A Strong Simmer": 991,
    "An Extreme Simmer": 990,
    "Fork Flickin' to the Summit": 296,
    "Fork Flickin' Detour": 297,
    "Excavate ‘n' Search the Cheese Rocks": 300,
    "Climb the Cheese Rocks": 301
}

loc_Luncheon_Post_Cheese_Rocks = {
    "Big Pot on the Volcano: Dive In!": 292,
    "Climb Up the Cascading Magma": 257,
    "Surrounded by Tall Mountains": 248,
    "Light the Lantern on the Small Island": 254,
    "Golden Turnip Recipe 3": 243,
    "Beneath the Rolling Vegetables": 263,
    "All the Cracks Are Fixed": 970,
    "Taking Notes: Swimming in Magma": 244,
    "Magma Narrow Path": 299,
    "Crossing to the Magma": 298,
    "Spinning Athletics End Goal": 307,
    "Taking Notes: Spinning Athletics": 306,
    "Luncheon Kingdom Timer Challenge 2": 249,
    "Treasure Beneath the Cheese Rocks": 952,
    "Light the Two Flames": 255,
    "The Treasure Chest in the Veggies": 992,
    "Found with Luncheon Kingdom Art": 1090,
    "Alcove Behind the Pillars of Magma": 259,
    "Cookatiel Showdown!": 290,
}

loc_Luncheon_Peace = {
    "Luncheon Kingdom Timer Challenge 3": 711,
    "Caught Hopping at the Volcano!": 289,
    "Light the Far-Off Lanterns": 261,
    "Bon Appetit, Captain Toad!": 265,
    "Taking Notes: Big Pot Swim": 833,
    "A Tourist in the Luncheon Kingdom!": 908
}

loc_Luncheon_Shop = {
    "Chef Hat": 2557,
    "Chef Suit": 2558,
    "Painter's Cap": 2559,
    "Painter Outfit": 2560,
    "Luncheon Kingdom Sticker": 2561,
    "Souvenir Forks": 2562,
    "Vegetable Plate": 2563,
    "Clown Hat": 2596,
    "Clown Suit": 2597
}

loc_Luncheon_Postgame = {
    "The Rooftop Lantern": 269,
    "Jammin' in the Luncheon Kingdom": 907,
    "Mechanic: Repairs Complete!": 886,
    "Diving from the Big Pot!": 1005,
    "Hat-and-Seek: Among the Food": 280,
    "Luncheon Kingdom: Regular Cup": 266,
    "Peach in the Luncheon Kingdom": 268,
    "From Inside a Bright Stone": 271,
    "Under the Meat Plateau": 829,
    "On Top of a Tall, Tall Roof": 287,
    "From a Crack in the Hard Ground": 276,
    "By the Cannon Pointed at the Big Pot": 277,
    "Luncheon Kingdom: Master Cup": 895,
    "Stepping Over the Gears and Lanterns on the Gear Steps": 313,
    "Lanterns on the Gear Steps": 312,
    "Volcano Cave Cruisin'": 310,
    "Volcano Cave and Mysterious Clouds": 311,
    "Treasure of the Lava Islands": 1101,
    "Flying Over the Lava Islands": 308
}

loc_Ruined = {
    "Battle with the Lord of Lightning!": 795,
    "In the Ancient Treasure Chest": 793,
    "Roulette Tower: Climbed": 888,
    "Roulette Tower: Stopped": 889
}

loc_Ruined_Postgame = {
    "Peach in the Ruined Kingdom": 1117,
    "Caught in a Big Horn": 834,
    "Upon the Broken Arch": 835,
    "Rolling Rock on the Battlefield": 909,
    "Charging Through an Army": 891,
    "The Mummy's Curse": 890
}

loc_Bowser = {
    "Infiltrate Bowser's Castle!": 325,
    "Behind the Big Wall": 321,
    "Caught on the Iron Fence": 345,
    "Taking notes: Between spinies": 315,
    "Stack up above the wall": 324,
}

loc_Bowser_Infiltrate = {
    "Smart Bombing": 334,
    "Treasure Inside the Turret": 333,
    "Poking Your Nose in the Plaster Wall": 323,
    "Poking the Turret Wall": 337,
}

loc_Bowser_Post_Bombing = {
    "Big Broodal Battle": 314,
    "From the Side above the Castle Gate": 335,
    "Exterminate the Ogres!": 320,
    "Jizo All in a Row": 316,
    "Underground Jizo": 317,
    "Shopping at Bowser's Castle": 360,
    "Scene of Crossing the Poison Swamp": 987,
    "Taking Notes: In the Folding Screen": 988,
}

loc_Bowser_Mecha_Broodal = {
    "Showdown at Bowser's Castle": 332,
}

loc_Bowser_Peace = {

    "Sunken Treasure in the Moat": 327,
    "On the Giant Bowser Statue's Nose": 339,
    "Inside a Block in the Castle": 319,
    "Hidden Corridor Under the Floor": 1092,
    "Poking Your Nose by the Great Gate": 328,
    "Found Behind Bars!": 331,
    "Good to See You, Captain Toad!": 318,
    "Dashing Above the Clouds": 365,
    "Dashing Through the Clouds": 366,
    "Past the Moving Wall": 340,
    "Above the Poison Swamp": 326,
    "Knocking Down the Nice Frame": 1133,
    "Caught Hopping at Bowser's Castle!": 343,
    "Bowser's Kingdom Timer Challenge 1": 691,
    "Fishing(?) in Bowser's Castle": 26,
    "Bowser's Castle Treasure Vault": 999,
    "On Top of the Spinning Tower": 368,
    "Down and Up the Spinning Tower": 367,
    "Jizo's Big Adventure": 363,
    "Jizo and the Hidden Room": 364,
}

loc_Bowser_Shop = {
    "Samurai Helmet": 2564,
    "Samurai Armor": 2565,
    "Happi Headband": 2566,
    "Happi Outfit": 2567,
    "Bowser's Kingdom Sticker": 2568,
    "Paper Lantern": 2569,
    "Jizo Statue": 2570
}

loc_Bowser_Postgame = {
    "Sphynx Over Bowser's Castle": 1039,
    "I Met a Pokio!": 941,
    "Bowser's Kingdom Regular Cup": 859,
    "A Rumble Under the Arena Floor": 1161,
    "Secret Path to Bowser's Castle": 322,
    "Peach in Bowser's Kingdom": 338,
    "Found with Bowser's Kingdom Art": 1091,
    "Behind the Tall Wall: Poke, Poke!": 350,
    "From Crates in the Moat": 1032,
    "Caught on the Giant Horn": 1016,
    "Inside a Block at the Gate": 356,
    "Small Bird in Bowser's Castle": 1014,
    "Invader in Bowser's Castle": 349,
    "Jumping from Flag to Flag": 355,
    "Bowser's Kingdom Timer Challenge 2": 52,
    "Taking Notes: On the Wall": 351,
    "Taking Notes with a Spinning Throw": 1102,
    "Third Courtyard Outskirts": 348,
    "Stone Wall Circuit": 1143,
    "Bowser's Kingdom Master Cup": 896,
    "Searching Hexagon Tower": 801,
    "Center of Hexagon Tower": 802,
    "Climb the Wooden Tower": 370,
    "Poke the Wooden Tower": 369
}

loc_Moon = {
    "Shining Above the Moon": 579,
    "Along the Cliff Face": 586,
    "The Tip of a White Spire": 577,
    "Rolling Rock on the Moon": 583,
    "Caught Hopping on the Moon!": 585,
    "Cliffside Treasure Chest": 582,
    "Moon Kingdom Timer Challenge 1": 584,
    "Taking Notes: On the Moon's Surface": 576,
    "Under the Bowser Statue": 595,
    "In a Hole in the Magma": 596,
    "Around the Barrier Wall": 597,
    "On Top of the Cannon": 594,
    "Fly to the Treasure Chest and Back": 598,
    "Up in the Rafters": 593,
}

loc_Moon_Shop = {
    "Space Helmet": 2571,
    "Space Suit": 2572,
    "Moon Kingdom Sticker": 2573,
    "Moon Rock Fragment": 2574,
    "Moon Lamp": 2575,
    "Football Helmet": 2598,
    "Football Uniform": 2599,
    "Mario's Top Hat": 2624,
    "Mario's Tuxedo": 2625
}

loc_Moon_Post_Moon = {
    "Beat the Game": 2500
}

loc_Moon_Postgame = {
    "Sneaking Around in the Crater": 693,
    "Found on the Moon, Good Dog!": 672,
    "Moon Shards on the Moon": 671,
    "Moon Quiz: Amazing!": 580,
    "Thanks, Captain Toad!": 1000,
    "Shopping in Honeylune Ridge": 1157,
    "Walking on the Moon!": 578,
    "Moon Kingdom Regular Cup": 707,
    "Doctor in the House": 1164,
    "Sphynx's Hidden Vault": 599,
    "A Tourist in the Moon Kingdom": 911,
    "Peach in the Moon Kingdom": 581,
    "Found with Moon Kindgom Art": 1165,
    "Mysterious Flying Object": 1049,
    "Hidden on the Side of the Cliff": 810,
    "Jumping High as a Frog": 811,
    "Moon Kingdom Timer Challenge 2": 798,
    "Walking on the Moon: Again!": 714,
    "Moon Kingdom Master Cup": 899,
    "Taking Notes: In Low Gravity": 828,
    "Center of the Galaxy": 603,
    "Edge of the Galaxy": 604,
    "Navigating Giant Swings": 601,
    "A Swing on Top of a Swing": 602
}

loc_Mushroom = {
    "A Tourist in the Mushroom Kingdom": 912,
    "Perched on the Castle Roof": 1134,
    "Pops out of the Tail": 1027,
    "Caught Hopping at Peach's Castle!": 613,
    "Gardening for Toad: Garden Seed": 1108,
    "Gardening for Toad: Field Seed": 1109,
    "Gardening for Toad: Pasture Seed": 1111,
    "Gardening for Toad: Lake Seed": 1110,
    "Grow a Flower Garden": 959,
    "Mushroom Kingdom Timer Challenge": 977,
    "Found at Peach's Castle! Good Dog!": 840,
    "Taking Notes: Around the Well": 1121,
    "Herding Sheep at Peach's Castle": 607,
    "Gobbling Fruit with Yoshi": 856,
    "Yoshi's Second Helping!": 857,
    "Yoshi's All Filled Up!": 858,
    "Love at Peach's Castle": 606,
    "Toad Defender": 984,
    "Forever Onward, Captain Toad!": 605,
    "Jammin' in the Mushroom Kingdom": 913,
    "Shopping Near Peach's Castle": 933,
    "Mushroom Kingdom Regular Cup": 967,
    "Mushroom Kingdom Master Cup": 968,
    "Picture Match: Basically Mario": 985,
    "Picture Match: A Stellar Mario!": 986,
    "Light from the Ceiling": 807,
    "Loose-Tile Trackdown": 956,
    "Totally Classic": 934,
    "Courtyard Chest Trap": 1144,
    "Yoshi's Feast in the Sea of Clouds": 974,
    "Sunken Star in the Sea of Clouds": 975,
    "Secret 2D Treasure": 1050,
    "2D Boost from Bullet Bill": 1051,
    "Tussle in Tostarena: Rematch": 1148,
    "Struggle in Steam Gardens: Rematch": 1149,
    "Dust-Up in New Donk City: Rematch": 1141,
    "Battle in Bubblaine: Rematch": 1147,
    "Blowup at Mount Volbono: Rematch": 1142,
    "Rumble in Crumbleden: Rematch": 1146,
    "Found with Mushroom Kingdom Art": 1152,
    "Hat-and-Seek: Mushroom Kingdom": 978,
    "Princess Peach, Home Again!": 1119,
}

loc_Mushroom_Post_Luncheon = {
    "Secret Path to Peach's Castle!": 608
}

loc_Mushroom_Shop = {
    "Mario 64 Cap": 2576,
    "Mario 64 Suit": 2577,
    "Mushroom Cushion Set": 2583,
    "Peach's Castle Model": 2584,
    "Pipe Sticker": 2578,
    "Coin Sticker": 2579,
    "Block Sticker": 2580,
    "? Block Sticker": 2581,
    "Mushroom Kingdom Sticker": 2582,
    "Classic Cap": 2600,
    "Classic Suit": 2601,
    "Skeleton Suit": 2602
}

loc_Postgame_Shop = {
    "Luigi Cap": 2603,
    "Luigi Suit": 2604,
    "Doctor Headwear": 2605,
    "Doctor Outfit": 2606,
    "Waluigi Cap": 2607,
    "Waluigi Suit": 2608,
    "Diddy Kong Hat": 2609,
    "Diddy Kong Suit": 2610,
    "Wario Cap": 2611,
    "Wario Suit": 2612,
    "Hakama": 2613,
    "Bowser's Top Hat": 2614,
    "Bowser's Tuxedo": 2615,
    "Bridal Veil": 2616,
    "Bridal Gown": 2617,
    "Gold Mario Cap": 2618,
    "Gold Mario Suit": 2619,
    "Metal Mario Cap": 2620,
    "Metal Mario Suit": 2621
}

loc_Dark = {
    "Arrival at Rabbit Ridge!": 1055,
    "Captain Toad on the Dark Side!": 1122,
    "Breakdown Road: Hurry": 1062,
    "Breakdown Road: Final Challenge!": 1063,
    "Invisible Road: Rush!": 1069,
    "Invisible Road: Secret!": 1068,
    "Vanishing Road Rush": 1065,
    "Vanishing Road Challenge": 1064,
    "Yoshi Under Siege": 1066,
    "Fruit Feast Under Siege": 1067,
    "Yoshi on the Sinking Island": 1070,
    "Fruit Feast on the Sinking Island": 1071,
    "Yoshi's Magma Swamp": 1082,
    "Fruit Feast in the Magma Swamp!": 1083,
    "Found with Dark Side Art 1": 1132,
    "Found with Dark Side Art 2": 1130,
    "Found with Dark Side Art 3": 1131,
    "Found with Dark Side Art 4": 1124,
    "Found with Dark Side Art 5": 1129,
    "Found with Dark Side Art 6": 1127,
    "Found with Dark Side Art 7": 1126,
    "Found with Dark Side Art 8": 1123,
    "Found with Dark Side Art 9": 1128,
    "Found with Dark Side Art 10": 1125,
}

loc_Dark_Outfit = {
    "King's Crown": 2622,
    "King's Outfit": 2623
}

loc_Darker = {
    "A Long Journey's End!": 1061,
}

loc_Darker_Outfit = {
    "Invisibility Hat": 2502
}

loc_Captures = {
    "Frog": 3701,
    "Spark Pylon": 3702,
    "Paragoomba": 3703,
    "Chain Chomp": 3704,
    "Big Chain Chomp": 3705,
    "Broode's Chain Chomp": 3706,
    "T-Rex": 3707,
    "Binoculars": 3708,
    "Bullet Bill": 3709,
    "Moe-Eye": 3710,
    "Cactus": 3711,
    "Goomba": 3712,
    "Knucklotec's Fist": 3713,
    "Mini Rocket": 3714,
    "Glydon": 3715,
    "Lakitu": 3716,
    "Zipper": 3717,
    "Cheep Cheep": 3718,
    "Puzzle Part (Lake Kingdom)": 3719,
    "Poison Piranha Plant": 3720,
    "Uproot": 3721,
    "Fire Bro": 3722,
    "Sherm": 3723,
    "Coin Coffer": 3724,
    "Tree": 3725,
    "Boulder": 3726,
    "Picture Match Part (Goomba)": 3727,
    "Tropical Wiggler": 3728,
    "Pole": 3729,
    "Manhole": 3730,
    "Taxi": 3731,
    "RC Car": 3732,
    "Ty-foo": 3733,
    "Shiverian Racer": 3734,
    "Cheep Cheep (Snow Kingdom)": 3735,
    "Gushen": 3736,
    "Lava Bubble": 3737,
    "Volbonan": 3738,
    "Hammer Bro": 3739,
    "Meat": 3740,
    "Fire Piranha Plant": 3741,
    "Pokio": 3742,
    "Jizo": 3743,
    "Bowser statue": 3744,
    "Parabones": 3745,
    "Banzai Bill": 3746,
    "Chargin' Chuck": 3747,
    "Bowser": 3748,
    "Letter": 3749,
    "Puzzle Part (Metro Kingdom)": 3750,
    "Picture Match Part (Mario)": 3751,
    "Yoshi": 3752
}

base_locations_table = {
    **loc_Cap,
    **loc_Cascade,
    **loc_Cascade_Revisit,
    **loc_Cascade_Post_Metro,
    **loc_Cascade_Post_Snow,
    **loc_Sand,
    **loc_Sand_Pyramid,
    **loc_Sand_Underground,
    **loc_Sand_Peace,
    **loc_Sand_Revisit,
    **loc_Lake,
    **loc_Lake_Post_Seaside,
    **loc_Wooded,
    **loc_Wooded_Post_Metro,
    **loc_Wooded_Post_Story1,
    **loc_Wooded_Peace,
    **loc_Cloud,
    **loc_Lost,
    **loc_Lost_Revisit,
    **loc_Metro,
    **loc_Metro_Post_Sand,
    **loc_Metro_Sewer_Access,
    **loc_Metro_Peace,
    **loc_Snow,
    **loc_Snow_Peace,
    **loc_Seaside,
    **loc_Seaside_Peace,
    **loc_Luncheon,
    **loc_Luncheon_Post_Wooded,
    **loc_Luncheon_Post_Spewart,
    **loc_Luncheon_Post_Cheese_Rocks,
    **loc_Luncheon_Peace,
    **loc_Ruined,
    **loc_Bowser,
    **loc_Bowser_Infiltrate,
    **loc_Bowser_Post_Bombing,
    **loc_Bowser_Mecha_Broodal,
    **loc_Bowser_Peace,
    **loc_Moon,
    **loc_Mushroom_Post_Luncheon
}

shop_locations_table = {
    **loc_Cap_Shop,
    **loc_Cascade_Shop,
    **loc_Sand_Shop,
    **loc_Lake_Shop,
    **loc_Wooded_Shop,
    **loc_Lost_Shop,
    **loc_Metro_Shop,
    **loc_Snow_Shop,
    **loc_Seaside_Shop,
    **loc_Luncheon_Shop,
    **loc_Bowser_Shop,
    **loc_Moon_Shop,
    **loc_Mushroom_Shop,
    **loc_Postgame_Shop,
    **loc_Dark_Outfit,
    **loc_Darker_Outfit
}

post_game_locations_table = {
    **loc_Cap_Postgame,
    **loc_Cascade_Postgame,
    **loc_Sand_Postgame,
    **loc_Lake_Postgame,
    **loc_Wooded_Postgame,
    **loc_Cloud_Postgame,
    **loc_Lost_Postgame,
    **loc_Metro_Postgame,
    **loc_Snow_Postgame,
    **loc_Seaside_Postgame,
    **loc_Luncheon_Postgame,
    **loc_Ruined_Postgame,
    **loc_Bowser_Postgame,
    **loc_Moon_Postgame,
    **loc_Mushroom
}

special_locations_table = {
    **loc_Dark,
    **loc_Darker
}

locations_table = {
    **base_locations_table,
    **post_game_locations_table,
    **special_locations_table,
    **shop_locations_table,
    **loc_Post_Cloud,
    **loc_Moon_Post_Moon
}

locations_list = [
    loc_Cap,
    {**loc_Cascade, **loc_Cascade_Revisit, **loc_Cascade_Post_Snow, **loc_Cascade_Post_Metro},
    {**loc_Sand, **loc_Sand_Peace, **loc_Sand_Revisit, **loc_Sand_Pyramid},
    {**loc_Lake, **loc_Lake_Post_Seaside},
    {**loc_Wooded, **loc_Wooded_Post_Story1, **loc_Wooded_Peace, **loc_Wooded_Post_Metro},
    loc_Cloud,
    {**loc_Lost, **loc_Lost_Revisit},
    {**loc_Metro, **loc_Metro_Sewer_Access, **loc_Metro_Peace, **loc_Metro_Post_Sand},
    {**loc_Snow, **loc_Snow_Peace},
    {**loc_Seaside, **loc_Seaside_Peace},
    {**loc_Luncheon, **loc_Luncheon_Post_Spewart, **loc_Luncheon_Post_Cheese_Rocks, **loc_Luncheon_Peace, **loc_Luncheon_Post_Wooded},
    loc_Ruined,
    {**loc_Bowser, **loc_Bowser_Infiltrate, **loc_Bowser_Post_Bombing, **loc_Bowser_Mecha_Broodal, **loc_Bowser_Peace},
    loc_Moon,
    {**loc_Mushroom, **loc_Mushroom_Post_Luncheon},
    loc_Dark,
    loc_Darker
]
full_moon_locations_list = [
    {**loc_Cap, **loc_Cap_Postgame},
    {**loc_Cascade, **loc_Cascade_Revisit, **loc_Cascade_Post_Snow, **loc_Cascade_Post_Metro, **loc_Cascade_Postgame},
    {**loc_Sand, **loc_Sand_Peace, **loc_Sand_Revisit, **loc_Sand_Pyramid, **loc_Sand_Underground, **loc_Sand_Postgame},
    {**loc_Lake, **loc_Lake_Post_Seaside, **loc_Lake_Postgame},
    {**loc_Wooded, **loc_Wooded_Post_Story1, **loc_Wooded_Peace, **loc_Wooded_Post_Metro, **loc_Wooded_Postgame},
    {**loc_Cloud, **loc_Cloud_Postgame},
    {**loc_Lost, **loc_Lost_Revisit, **loc_Lost_Postgame},
    {**loc_Metro, **loc_Metro_Sewer_Access, **loc_Metro_Peace, **loc_Metro_Post_Sand, **loc_Metro_Postgame},
    {**loc_Snow, **loc_Snow_Peace, **loc_Snow_Postgame},
    {**loc_Seaside, **loc_Seaside_Peace, **loc_Seaside_Postgame},
    {**loc_Luncheon, **loc_Luncheon_Post_Spewart, **loc_Luncheon_Post_Cheese_Rocks, **loc_Luncheon_Peace, **loc_Luncheon_Post_Wooded, **loc_Luncheon_Postgame},
    {**loc_Ruined, **loc_Ruined_Postgame},
    {**loc_Bowser, **loc_Bowser_Infiltrate, **loc_Bowser_Post_Bombing, **loc_Bowser_Mecha_Broodal, **loc_Bowser_Peace, **loc_Bowser_Postgame},
    {**loc_Moon, **loc_Moon_Postgame},
    {**loc_Mushroom, **loc_Mushroom_Post_Luncheon},
    loc_Dark,
    loc_Darker
]

post_game_locations_list = [
    loc_Cap_Postgame,
    loc_Cascade_Postgame,
    loc_Sand_Postgame,
    loc_Lake_Postgame,
    loc_Wooded_Postgame,
    loc_Cloud_Postgame,
    loc_Lost_Postgame,
    loc_Metro_Postgame,
    loc_Snow_Postgame,
    loc_Seaside_Postgame,
    loc_Luncheon_Postgame,
    loc_Ruined_Postgame,
    loc_Bowser_Postgame,
    loc_Moon_Postgame
]

goals_table = {
    4 : "The Hole in the Desert",
    5 : "Broodals Over the Lake",
    9 : "A Traditional Festival!",
    12 : "Cookatiel Showdown!",
    14 : "Beat the Game",
    16 : "Arrival at Rabbit Ridge!",
    17 : "A Long Journey's End!"
}